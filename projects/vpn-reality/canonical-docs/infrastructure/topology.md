...
Топология текущей инсталляции VPN: один зарубежный VPS с 3x-ui/Xray, VLESS Reality + Vision flow на :443, два клиента.
...
- 3x-ui панель: порт `29486`, WebBasePath `/CBBrgsrNZfY43jtrAY/`, слушает на всех интерфейсах. Порт 29486 в ufw закрыт — доступ только через SSH-туннель. Subscription server 3x-ui на порту `2096` (не используется пока). Xray core: 26.2.6. 3x-ui: 2.8.11.
...
- 3x-ui не умеет подставлять внешний IP в share-ссылки автоматически — берёт хост из URL, через который открыта панель. Поскольку панель открывается через SSH-туннель на `127.0.0.1:29486`, все экспортируемые ссылки содержат `127.0.0.1` — нужно вручную заменять на `147.45.196.137` при копировании.
