---
date: 2026-04-12
recorded_at: 2026-04-12T15:05:06.144Z
project: vpn-reality
...
- На DE открыть 3x-ui через SSH runbook `panel-access` и создать отдельный inbound `relay-backhaul` на `8443/tcp`: `VLESS`, `TCP`, `Reality`, `xtls-rprx-vision`, `dest/serverName = www.microsoft.com:443`, `fingerprint = chrome`, с отдельным UUID для relay.
...
- На Yandex relay создать `/usr/local/etc/xray/config.json` со схемой: inbound `relay-public` на `0.0.0.0:443` (`VLESS + Reality + Vision`) и outbound `de-backhaul` на `147.45.196.137:8443` (`VLESS + Reality + Vision`). В routing весь трафик inbound `relay-public` отправлять в outbound `de-backhaul`.
...
- Собрать и раздать новую share-ссылку на Yandex IP с relay public key и short-id. Старый прямой DE endpoint не использовать как primary path для новых устройств.
