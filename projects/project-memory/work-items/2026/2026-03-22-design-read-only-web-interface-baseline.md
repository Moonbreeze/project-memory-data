...
Define the delivery architecture and first-version scope for a read-only Web UI over project-memory managed documents.

## Outcome

Project-memory has an explicit baseline design for a human-facing read-only Web UI, including the chosen runtime shape, shared-core data flow, v1 feature boundary, configuration model, and the next implementation slices that can proceed without reopening the architecture.
...
- Decide whether search is part of v1; if not, record it as an explicit follow-up rather than leaving it ambiguous.
