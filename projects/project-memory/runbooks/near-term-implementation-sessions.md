...
- Session 3, read entrypoints phase 1: implement the first bounded read flows for cold start, topic lookup, and rationale lookup so agents can reach the current truth without broad project scans.
- Session 4, work-item spec: finalize the future work-item schema, lifecycle, provenance model, dependency metadata, planning states, and relationships to decisions, canonical docs, session notes, and verification results.
- Session 5, work-item implementation: implement the first work-item document type and the initial backlog or planning flows, including dependency-aware ready versus blocked interpretation and basic close or archive semantics.
- Session 6, lifecycle and cross-project helpers: add the next layer of lifecycle operations, partial migration helpers, and explicit cross-project link handling after the core document types and read flows are stable.
